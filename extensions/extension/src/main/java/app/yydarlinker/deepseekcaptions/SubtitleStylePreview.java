...
public final class SubtitleStylePreview extends android.preference.Preference {
    ...
    @Override protected View onCreateView(ViewGroup parent){
        ...
        LinearLayout pair=new LinearLayout(getContext());pair.setGravity(Gravity.CENTER_VERTICAL);
        Preview wide=new Preview(getContext());views.add(wide);Preview tall=new Preview(getContext());tall.portrait=true;views.add(tall);
        LinearLayout.LayoutParams left=new LinearLayout.LayoutParams(0,-2,1.65f);left.setMarginEnd(pad/2);
        pair.addView(wide,left);pair.addView(tall,new LinearLayout.LayoutParams(0,-2,1f));root.addView(pair,new LinearLayout.LayoutParams(-1,-2));
        TextView hint=new TextView(getContext());hint.setText("横屏 / Shorts · 共用字号和透明度");hint.setTextSize(12);hint.setAlpha(.65f);hint.setPadding(0,pad/3,0,0);
    ...
    static final class Preview extends View {
        ...
        @Override protected void onMeasure(int w,int h){int width=MeasureSpec.getSize(w);setMeasuredDimension(width,(int)(portrait?width*16f/9f:width*9f/16f));}
