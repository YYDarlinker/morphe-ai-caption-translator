...
public class SegmentationR7Test {
 ...
 @Test public void presentationOverflowDoesNotCreatePaidRetry()throws Exception{
 ...
 AnchoredCaptionPlan p=AnchoredCaptionPlan.parse(new org.json.JSONArray().put(new org.json.JSONArray().put(u.toAtom-u.fromAtom).put(text)),a,u);
 assertEquals(text,p.canonical);assertFalse(CaptionSegmentationPolicy.issue(p.segments.get(0)).isEmpty());
 }
