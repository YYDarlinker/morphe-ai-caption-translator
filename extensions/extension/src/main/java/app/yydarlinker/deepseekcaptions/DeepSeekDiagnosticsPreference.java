...
public final class DeepSeekDiagnosticsPreference extends android.preference.Preference {
    ...
    @Override protected View onCreateView(ViewGroup parent){
        ...
        TextView title=new TextView(c);title.setText("字幕诊断");CaptionSettingsStyle.title(title);header.addView(title,new LinearLayout.LayoutParams(0,-2,1));
        Button toggle=new Button(c,null,android.R.attr.borderlessButtonStyle);CaptionSettingsStyle.button(toggle);toggle.setText("展开");toggle.setTag("ai_diagnostics_toggle");header.addView(toggle);box.addView(header);
        TextView hint=new TextView(c);hint.setText("排查翻译或时间问题时，可展开查看并复制记录");CaptionSettingsStyle.caption(hint);box.addView(hint);
        ...
        Button refresh=new Button(c,null,android.R.attr.borderlessButtonStyle);CaptionSettingsStyle.button(refresh);refresh.setText("刷新");refresh.setOnClickListener(v->{body.setText(CaptionDiagnostics.uiText(c));scroll.scrollTo(0,0);});actions.addView(refresh);
        Button copy=new Button(c,null,android.R.attr.borderlessButtonStyle);CaptionSettingsStyle.button(copy);copy.setText("复制");copy.setOnClickListener(v->{ClipboardManager manager=(ClipboardManager)c.getSystemService(Context.CLIPBOARD_SERVICE);if(manager!=null){manager.setPrimaryClip(ClipData.newPlainText("AI 字幕诊断",body.getText()));Toast.makeText(c,"诊断已复制",Toast.LENGTH_SHORT).show();}});actions.addView(copy);
        expanded.addView(scroll,new LinearLayout.LayoutParams(-1,CaptionSettingsStyle.dp(c,280)));box.addView(expanded);
        toggle.setOnClickListener(v->{boolean open=expanded.getVisibility()!=View.VISIBLE;if(open)body.setText(CaptionDiagnostics.uiText(c));expanded.setVisibility(open?View.VISIBLE:View.GONE);toggle.setText(open?"收起":"展开");toggle.setContentDescription(open?"收起字幕诊断":"展开字幕诊断");});return box;
    }
}
