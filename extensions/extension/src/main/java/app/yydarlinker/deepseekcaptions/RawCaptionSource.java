...
final class RawCaptionSource {
    ...
    static Source load(Context context,String translatedUrl,boolean publishSharedTimeline,boolean calibrate) throws Exception {
        ...
        if(calibrate && identity.autoGenerated && SourceAtomTimeline.build(body,document).preciseRatio()<0.8){
            try{
        ...
        if (identity.autoGenerated) {
            CaptionDiagnostics.mark(
                    ...
                    "当前底层轨为自动生成字幕；句段边界保留，内部非原生词时刻为估计；native="+SourceAtomTimeline.build(body,document).nativeTimedAtoms+";estimated="+SourceAtomTimeline.build(body,document).estimatedAtoms
            );
