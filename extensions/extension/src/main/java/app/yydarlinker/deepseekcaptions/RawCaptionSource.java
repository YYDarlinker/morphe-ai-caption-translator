...
final class RawCaptionSource {
    ...
    static Source load(Context context,String translatedUrl,boolean publishSharedTimeline,boolean calibrate) throws Exception {
        ...
        if(calibrate && identity.autoGenerated && providerAtoms.preciseRatio()<0.8){
            try{
                TimingAnchor precise=loadAsrAnchor(context,sourceUrl,document);
                if(precise!=null){
                    SourceAtomTimeline.Result original=providerAtoms,reference=SourceAtomTimeline.build(precise.body,precise.document);
        ...
        if (identity.autoGenerated) {
            ...
            try {
                TimingAnchor anchor = loadAsrAnchor(context, sourceUrl,document);
                if (anchor == null) {
                    CaptionDiagnostics.mark(
                            context,
    ...
    private static TimingAnchor loadPublicWordAnchor(Context context,String sourceUrl,CaptionDocument.Parsed source,int budgetMs)throws Exception{
        ...
        synchronized(WORD_ANCHORS){TimingAnchor cached=WORD_ANCHORS.get(cacheKey);if(cached!=null&&matches(cached,source))return cached;}
        ...
        TimingAnchor found=loadMatchingAnchor(context,tracks,source,deadline,"ASR_WORD_REFERENCE");
        if(found!=null)synchronized(WORD_ANCHORS){if(WORD_ANCHORS.size()>=4)WORD_ANCHORS.remove(WORD_ANCHORS.keySet().iterator().next());WORD_ANCHORS.put(cacheKey,found);}
        return found;
    }
    private static boolean matches(TimingAnchor anchor,CaptionDocument.Parsed source){
        if(WordTimingReference.sameWords(source,anchor.document))return true;
        SourceAtomTimeline.Result original=SourceAtomTimeline.build(new byte[0],source);
        SourceAtomTimeline.Result reference=SourceAtomTimeline.build(anchor.body,anchor.document);
        return AsrLocalTiming.alignWithCueTiming(original,reference)!=original || CaptionTimingCalibrator.compare(source,anchor.document).anchors>=6;
    }
    private static TimingAnchor loadMatchingAnchor(Context context,List<String> urls,CaptionDocument.Parsed source,long deadline,String diagnostic){
        for(String url:urls){
            int remaining=(int)(deadline-android.os.SystemClock.elapsedRealtime());if(remaining<=0)break;
            try{
                LoadedTrack data=loadTrack(context,SourceFormatPolicy.json3(url),diagnostic,false,Math.min(3000,remaining));
                CaptionDocument.Parsed doc=CaptionDocument.parse(data.body,data.contentType);
                if(doc.cues().isEmpty())continue;
                TimingAnchor anchor=new TimingAnchor(data.body,data.contentType,url,doc);
                if(matches(anchor,source))return anchor;
                CaptionDiagnostics.mark(context,"ASR_REFERENCE_REJECTED","reason=source_text_mismatch;lang="+query(url,"lang"));
            }catch(Exception failure){CaptionDiagnostics.mark(context,"ASR_REFERENCE_FETCH_FAILED",diagnostic+";reason="+timingFailureReason(failure));}
        }
        return null;
    }
    private static TimingAnchor loadAsrAnchor(Context context,String sourceUrl,CaptionDocument.Parsed source)throws Exception{
        ...
        TimingAnchor best=loadMatchingAnchor(context,NativeAsrTrackReference.candidates(video,sourceLanguage(sourceUrl,source)),source,deadline,"ASR_NATIVE_TRACK");
        // A matching native ASR cue clock is already usable. Do not fetch a watch page on
        // the first-caption critical path merely because it has no per-word offsets.
        if(best!=null)return best;
        int remaining=(int)(deadline-android.os.SystemClock.elapsedRealtime());
        if(remaining>0)try{
            TimingAnchor found=loadPublicWordAnchor(context,sourceUrl,source,Math.min(5000,remaining));
        ...
        if(best!=null)return best;
        return loadMatchingAnchor(context,autoGeneratedCandidates(sourceUrl,sourceLanguage(sourceUrl,source)),source,deadline,"ASR_TIMING");
    }
